...
public class C04_Method {
    ...
    public static void dogrucevapsayisi(char[][] answers, char[] keys) {
        ...
        for (int i = 0; i < answers.length; i++) {//0'dan ogrenci syisina kadar donguye aldik

            int sayac=0;
            for (int j = 0; j < answers[i].length; j++) {//0'dan ogrencinin cevap sayisina kadar donguye alalim

                if (answers[i][j]==keys[j]){//ogrencini cevabi cevap anahtari ile esit mi kontrolu
                    sayac++;
                }

            }
            System.out.println(i+1 + " nolu ogrencini dogru cevap sayisi : " + sayac);
        }
    }
}
